...
Модель разрушения использует три основных параметра, управляемых с помощью слайдеров. Они определяют поведение материала под нагрузкой и позволяют настраивать физическую реалистичность модели.

---

### 1. Пиковое напряжение (σ₀, `sigma0`)  
- Диапазон: 1 – 100  
- Единицы измерения: условные (относительная величина)

Это значение задаётся вручную при клике по ячейке. Оно обозначает локальную нагрузку или внешний источник напряжения. Чем выше σ₀, тем больше исходное напряжение, распространяющееся по сетке.

```js
grid[y][x].stress = sigma0Slider.value(); // при клике по ячейке
```
## 2. Коэффициент затухания (decay)
- Диапазон: 0.01 – 0.5
- Единицы измерения: 1/ячейка
Определяет скорость убывания напряжения с расстоянием. Используется в формуле:
```js
σ(x, y) += σ₀ * e^(−decay * d) / (1 + d²)
```
где d — расстояние между ячейками, σ₀ — напряжение источника.
Большие значения decay означают, что напряжение почти не распространяется далеко от источника.

## 3. Коэффициент разрушения (fₛ, fS)
- Диапазон: 0.75 – 0.98
- Единицы измерения: безразмерная величина (доля от max stress)
Этот коэффициент определяет порог разрушения:
```js
если σ(x, y) ≥ fₛ × σₘₐₓ → начинается накопление повреждения
```
- σ(x, y) — напряжение в ячейке
- σₘₐₓ — максимальное напряжение на текущей итерации по всей сетке
Ячейка разрушается, когда damage ≥ 1

## Дополнительные элементы модели
- Повреждение (damage)
- Накапливается по +0.3, если напряжение выше порога (fS × σₘₐₓ)
- Уменьшается на −0.05, если напряжение меньше порога
- Позволяет моделировать усталостное разрушение
